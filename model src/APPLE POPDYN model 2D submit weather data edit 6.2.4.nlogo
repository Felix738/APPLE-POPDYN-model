...
globals [
  ...
  PAR-data  ; real climate data in relation to photosynthetically active radiation
...
; read in climate data                                                                                                              ; this rectangle determines the main stem
  set PAR-data []                 ;; start with empty list
  file-open "PAR_data_1937_one_col.txt"        ;; open the file

  while [not file-at-end?] [
    set PAR-data lput file-read PAR-data  ;; read one number, append to list
  ]
...
; in addition this code chunk makes it possible to read in active solar radiation measurements
  let i (ticks mod y)  ;; assuming one tick = one measurement (half-week)
  if i < length PAR-data and (Read-in-real-weather-data?) [
    set new_PARI item i PAR-data
  ]
end
...
to net-assimilation
  ...
  ask leaves [set net_assimilation phot_limit_rad]  ; phot_limit_CO2 taken out of equation but originally included
  ; ask leaves [set net_assimilation min (list phot_limit_rad phot_limit_wat_pot)]
...
to exporting
  export-plot "Mean net assimilation" "output files/Mean net assimilation"  ; exporting plots to csv files
...
;  export-world "output files/APPLE POPDYN model 2D submit weather data.csv"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot new_PARI"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count leaf_miners"
